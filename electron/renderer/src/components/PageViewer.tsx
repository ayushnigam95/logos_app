...
import { getTranslatedPage, exportPagePdf, getPageSummary, getPageNotes, analyzeImage } from '../services/api';
...
export function PageViewer({ jobId, pageId }: Props) {
  ...
  const handleDownloadPdf = async () => {
    setPdfBusy(true);
    try {
      await exportPagePdf(jobId, pageId);
    } catch (e) {
      ...
      {activePanel && (
        <div className={`page-panel page-panel-${activePanel}`}>
          ...
          {panelLoading && !panelContent[activePanel] ? (
            ...
            <Markdown source={panelContent[activePanel] ?? ''} />
          )}
        </div>
      )}
      <div
